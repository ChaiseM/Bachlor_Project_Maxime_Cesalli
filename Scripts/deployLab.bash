...
#
# Example usage 19: ./Scripts/deployLab.bash -v -p eln-kart -r https://github.com/hei-synd-2131-eln/eln-kart.git
#
# Example usage 20: ./Scripts/deployLab.bash -v -p eln-display -r https://github.com/hei-isc-eln/eln-display.git
# Example usage 21: ./Scripts/deployLab.bash -v -p isc-eln-labs -r https://github.com/hei-isc-eln/eln-labs.git
#
...
# [isc-eln-labs](https://github.com/hei-isc-eln/eln-labs)
# [ele_labs]()
...
# [eln_display](https://github.com/hei-isc-eln/eln-display.git)
# [eln_support]()
# [eptm_radio]()
# [eptm_audioamp]()
# [cansat]()
...
if [ "$project" == "synd_eln_labs" ]; then
  ...
  cd eln_labs
elif [ "$project" == "isc-eln-labs" ]; then
  cd eln-labs
else
  cd $project
fi
...
if [ "$project" == "synd_eln_labs" ]; then
  ...
  echo ""
  echo "Delete solutions blocs for eln-kart"
  ...
  rm -v -f -r ./Board/concat/concatenated.vhd
  rm -v -f -r ./Board/concat/Kart.vhd
  find ./Board/libero/Kart/designer/ ! -name "*.prjx" -type f -exec rm -f {} +
  ...
  echo ""
  echo "Delete solutions blocs for eln_synchro"
  rm -v -f -r ./Board/concat/concatenated.vhd
  rm -v -f -r ./Board/concat/synchro.vhd
  ...
  rm -v -f -r ./Board/concat/concatenated.vhd
  rm -v -f -r ./Board/concat/inverter.vhd

elif [ "$project" == "eln-display" ]; then
  find $repo_source -maxdepth 1 -type f \! \( -name .gitmodules -o -name .gitlab-ci.yml -o -name README.md -o -name LICENSE \) -exec cp -ar '{}' $repo_dest \;
  for folder in $(find $repo_source -maxdepth 1 -type d )
  do
    if [[ "$repo_source" != "$folder" && "$repo_source/.git" != "$folder" && "$repo_source/Libs" != "$folder" && "$repo_source/Scripts" != "$folder" && "$repo_source/img" != "$folder" ]] ; then
  ...
  echo ""
  echo "Delete solutions blocs for eln-display"
  rm -v -f -r ./Board/concat/concatenated.vhd
  rm -v -f -r ./Board/concat/eln-display.vhd
...
if [ "$project" == "synd_eln_labs" ]; then
  ...
  ./../../changeDefaultViews.bash -p "Scripts/tmp/eln-labs" -a masterVersion -n studentVersion -r
  ./../../changeDefaultViews.bash -p "Scripts/tmp/eln-labs" -a master@version -n student@version -r
else
